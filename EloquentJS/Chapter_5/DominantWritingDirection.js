...
const characterScript = (characterCode) => {
    for(let script of SCRIPTS){
        const scriptMatch = script.ranges.some(([from, to])=>{
...
const dominantWritingDirection = (str) => {
    ...
    for(let i = 0; i< str.length; i++){
        ...
        const script = characterScript(charCode);
        if(script)
            scriptsDetected.push(script);
    }
    const counted = countBy(scriptsDetected, script => script.direction);
